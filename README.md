...
* [`src/cljs/safka_front/`](src/cljs/safka_front/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/safka_front/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `safka-front` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/safka_front/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns safka-front.example
  (:require [safka-front.config :as config])
